...
def _always_on(basename: str) -> str:
    ...
    This Codex-only fork ships one always-on block (AGENTS.md). It lives as
    committed markdown next to this module, generated by tools/skillgen from a
    single human-edited fragment and guarded against drift by
    ``skillgen --check``. The installer injects it verbatim via
...
# Dreamliner / Codex-only public install surface. Other host keys stay in
# `_PLATFORM_CONFIG` so leftover uninstall can still find their destinations.
_PUBLIC_INSTALL_PLATFORMS = frozenset({"codex"})
_DEFAULT_PLATFORM = "codex"
_GATED_HOST_COMMANDS = frozenset({
    "agents",
    "aider",
    "amp",
    "antigravity",
...
def _codex_only_error(platform_name: str) -> NoReturn:
    print(
        f"error: this graphify fork is Codex-only (got {platform_name!r}). "
        "Use `graphify install` or `graphify install --platform codex`.",
        file=sys.stderr,
    )
    sys.exit(1)


def _require_public_platform(platform_name: str) -> str:
    """Reject non-Codex install targets on this fork's public surface."""
    name = _canonical_platform(platform_name)
    if name not in _PUBLIC_INSTALL_PLATFORMS:
        _codex_only_error(name)
    return name
...
""")
        ...
        pass
def install(platform: str = _DEFAULT_PLATFORM, *, project: bool = False, project_dir: Path | None = None) -> None:
    _print_banner()
    platform = _require_public_platform(platform)
    if platform == "gemini":
...
def _print_install_usage() -> None:
    ...
    print("This Codex-only fork installs the Codex skill.")
    print("`graphify install` with no --platform targets Codex.")
    print("  --project         install into the current repository instead of the user profile")
    print("  --platform codex  explicit Codex target (the only supported platform)")
...
def _project_install(platform_name: str, project_dir: Path | None = None, strict: bool = False) -> None:
    ...
    project_dir = project_dir or Path(".")
    platform_name = _require_public_platform(platform_name)
...
def dispatch_install_cli(cmd: str) -> bool:
    ...
    if cmd not in _CLI_INSTALL_COMMANDS:
        return False
    if cmd in _GATED_HOST_COMMANDS:
        _codex_only_error(cmd)
    if cmd == "install":
        # This Codex-only fork always defaults to Codex, including on Windows.
        ...
        if project_scope:
            ...
            if strict:
                print(
                    "note: --strict is not used on Codex. AGENTS.md is the always-on mechanism.",
                    file=sys.stderr,
                )

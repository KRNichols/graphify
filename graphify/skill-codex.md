---
name: dreamliner
description: "Use for any question about a codebase, its architecture, file relationships, or project content — especially when graphify-out/ exists, where the question should be treated as a Dreamliner query first. Turns any input (code, docs, papers, images, videos) into a persistent knowledge graph with god nodes, community detection, and query/path/explain tools. Codex-only."
---
...
```
$dreamliner                                             # full pipeline on current directory (HTML viz; add --obsidian for a vault)
$dreamliner <path>                                      # full pipeline on specific path
$dreamliner https://github.com/<owner>/<repo>           # clone repo then run full pipeline on it
$dreamliner https://github.com/<owner>/<repo> --branch <branch>  # clone a specific branch
$dreamliner <url1> <url2> ...                           # clone multiple repos, build each, merge into one cross-repo graph
$dreamliner <path> --mode deep                          # thorough extraction, richer INFERRED edges
$dreamliner <path> --update                             # incremental - re-extract only new/changed files
$dreamliner <path> --directed                            # build directed graph (preserves edge direction: source→target)
$dreamliner <path> --whisper-model medium                # use a larger Whisper model for better transcription accuracy
$dreamliner <path> --cluster-only                       # rerun clustering on existing graph
$dreamliner <path> --no-viz                             # skip visualization, just report + JSON
$dreamliner <path> --html                               # (HTML is generated by default - this flag is a no-op)
$dreamliner <path> --svg                                # also export graph.svg (embeds in Notion, GitHub)
$dreamliner <path> --graphml                            # export graph.graphml (Gephi, yEd)
$dreamliner <path> --neo4j                              # generate graphify-out/cypher.txt for Neo4j
$dreamliner <path> --neo4j-push bolt://localhost:7687   # push directly to Neo4j
$dreamliner <path> --falkordb                           # generate graphify-out/cypher.txt for FalkorDB
$dreamliner <path> --falkordb-push falkordb://localhost:6379   # push directly to FalkorDB
$dreamliner <path> --mcp                                # start MCP stdio server for agent access
$dreamliner <path> --watch                              # watch folder, auto-rebuild on code changes (no LLM needed)
$dreamliner <path> --wiki                               # build agent-crawlable wiki (index.md + one article per community)
$dreamliner <path> --obsidian --obsidian-dir ~/vaults/my-project  # write vault to custom path (e.g. existing vault)
$dreamliner add <url>                                   # fetch URL, save to ./raw, update graph
$dreamliner add <url> --author "Name"                   # tag who wrote it
$dreamliner add <url> --contributor "Name"              # tag who added it to the corpus
$dreamliner query "<question>"                          # BFS traversal - broad context
$dreamliner query "<question>" --dfs                    # DFS - trace a specific path
$dreamliner query "<question>" --budget 1500            # cap answer at N tokens
$dreamliner path "AuthModule" "Database"                # shortest path between two concepts
$dreamliner explain "SwinTransformer"                   # plain-language explanation of a node
```

## What Dreamliner is for

Drop any folder of code, docs, papers, images, or video into Dreamliner and get a queryable knowledge graph. Persistent across sessions, honest audit trail (EXTRACTED/INFERRED/AMBIGUOUS), community detection surfaces cross-document connections you wouldn't think to ask about.

## What You Must Do When Invoked

If the user invoked `$dreamliner --help` or `$dreamliner -h` (with no other arguments), print the contents of the `## Usage` section above verbatim and stop. Do not run any commands, do not detect files, do not default the path to `.`. Just print the Usage block and return.

**Fast path — existing graph:** Before doing anything else, check whether `graphify-out/graph.json` exists. The expected location is `graphify-out/graph.json` relative to the **current working directory** (i.e. the project root where you are running commands). If it exists AND the user's request is a natural-language question about the codebase (e.g. "How does X work?", "What calls Y?", "Trace the data flow through Z") and NOT an explicit rebuild command (`--update`, `--cluster-only`, or a bare path/URL that implies fresh extraction): **skip Steps 1–5 entirely and jump straight to `## For $dreamliner query`.** Run `dreamliner query "<question>"` immediately. Do not run detect. Do not check corpus size. Do not ask the user to narrow. The graph is already built — use it.
...
> **Dreamliner needs no API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `$dreamliner .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses Codex `spawn_agent` workers. Do not prompt for, wait on, or stop because of a missing provider key — that is a misread of this skill.

> If `spawn_agent` is unavailable, tell the user to set `multi_agent = true` under `[features]` in `~/.codex/config.toml` and restart Codex. A code-only corpus can continue without subagents: write the empty semantic file (Part B "Fast path") and continue to Part C. For a corpus with docs/papers/images, extract those inline in this session if subagents cannot run — do not stall and do not ask for an API key.
...
**MANDATORY: You MUST dispatch Codex `spawn_agent` workers here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If `spawn_agent` is unavailable, tell the user to set `multi_agent = true` in `~/.codex/config.toml` and restart Codex.**
...
- If the file is missing, the subagent was likely dispatched as read-only (Explore type) — print a warning: "chunk N missing from disk — subagent may have been read-only. Re-run the chunk with a worker agent." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure Codex `spawn_agent` workers are used (not a read-only explore agent).

Merge all chunk files into `.graphify_semantic_new.json`. **After each Codex worker completes, record any available token counts and write them back into the in-memory merge before writing `.graphify_semantic_new.json`.** Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
...
```bash
dreamliner export obsidian
# or with custom dir: dreamliner export obsidian --dir ~/vaults/my-project
```

Generate the HTML graph (always, unless `--no-viz`):

```bash
dreamliner export html  # auto-aggregates to community view if graph > 5000 nodes
# or: dreamliner export html --no-viz
```
...
Dreamliner is a Codex-only fork of Graphify. The graph engine is unchanged.
...
If the user says yes, run `$dreamliner query "[question]"` on the graph and walk them through the answer using the graph structure - which nodes connect, which community boundaries get crossed, what the path reveals. Keep going as long as they want to explore. Each answer should end with a natural follow-up ("this connects to X - want to go deeper?") so the session feels like navigation, not a one-shot report.
...
## For $dreamliner add and --watch

Neither is part of the default build. When the user runs `$dreamliner add <url>` to fetch a URL into the corpus, or passes `--watch` to auto-rebuild on file changes, see `references/add-watch.md`.

---

## For the commit hook and native AGENTS.md integration

When the user asks to install the post-commit auto-rebuild hook or wire Dreamliner into a project's AGENTS.md, see `references/hooks.md`.
...
- If the CLI, graph.json, or Codex `spawn_agent` is missing or fails, surface the error and stop. Do not invent nodes, edges, or query answers.

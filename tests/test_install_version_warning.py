...
def test_the_warning_names_the_stale_destination_and_the_exact_command(tmp_path, capsys):
    mainmod._check_skill_version(_stale(tmp_path), platform_names=["codex"])
    ...
    assert "refreshes the Codex skill" in err
...
def test_a_current_skill_stays_silent(tmp_path, capsys):
    ...
    mainmod._check_skill_version(skill, platform_names=["codex"])
...
def test_a_locally_edited_skill_is_backed_up_and_announced(sandbox_home, capsys):
    _copy_skill_file("codex")
    dst = _platform_skill_destination("codex")
    assert dst.exists()

...
If `code_only` is True: print `[dreamliner update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely (no subagents), then go straight to merge and Steps 4–8.
...
if [ ! -f graphify-out/.graphify_extract.json ]; then
    echo '[dreamliner update] Only deletions -- creating empty extraction for merge.'
    $(cat graphify-out/.graphify_python) -c "
import json
from pathlib import Path
...
print(f'[dreamliner update] Merged: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges')
...
print(f'[dreamliner update] Merged extraction written ({len(merged_out[\"nodes\"])} nodes, {len(merged_out[\"edges\"])} edges)')
...
print('[dreamliner update] Manifest saved.')
"
```

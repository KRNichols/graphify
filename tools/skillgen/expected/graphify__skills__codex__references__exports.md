...
```bash
dreamliner export neo4j
```

**If `--neo4j-push <uri>`** - push directly to a running Neo4j instance. Ask the user for credentials if not provided:

```bash
dreamliner export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
```bash
dreamliner export falkordb --push falkordb://localhost:6379
```
...
```bash
dreamliner export svg
```

### Step 7c - GraphML export (only if --graphml flag)

```bash
dreamliner export graphml
```
...
To configure an MCP client, add to `claude_desktop_config.json`. Some MCP hosts can't run `$(...)`, and under `uv tool install` the system `python3` can't import graphify — so set `command` to the **absolute interpreter path** printed by `cat graphify-out/.graphify_python`:
```json
{
  "mcpServers": {
    "dreamliner": {
